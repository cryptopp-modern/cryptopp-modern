...
**Compatibility note:**
- `DL_DecryptorBase::Decrypt(RandomNumberGenerator& rng, ...)` now requires a functional RNG (blinding uses randomness). Callers must provide a real `RandomNumberGenerator` (for example `AutoSeededRandomPool`) rather than `NullRNG`.
